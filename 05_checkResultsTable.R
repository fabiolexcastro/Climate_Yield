...
order_table <- function(tb, nm){
  ...
  gg <- ggplot(data = gt %>% filter(var == 'spei'), aes(x = date, y = value, color = var)) +
    geom_line(size = 1.05) +
  ...
  ggsave(plot = gg, filename = paste0('./workspace/Yield_SPEI/png/graphs/', 'vars_', nm, '_scale_12.png'), units = 'in', width = 11, height = 7, dpi = 300)
  print('Done!')
  return(gt)
  
}
make_graph_yield <- function(tb){
  ...
  ggsave(plot = gg, filename = './workspace/Yield_SPEI/png/graphs/rdto_el_aguila.png', units = 'in', width  = 7, height = 5, dpi = 300)
  print('Done!')
  return(gg)
...
# Load data ---------------------------------------------------------------
pit <- readRDS('./workspace/Yield_SPEI/rds/mpios_vars/tb_pit_03.rds')
agu <- readRDS('./workspace/Yield_SPEI/rds/mpios_vars/tb_agu_03.rds')
...
pit <- pit %>% mutate(spei_log = log10(yield))
pit_gth <- pit %>% dplyr::select(year, -periodo, yield, spei_log)
pit_gth <- pit_gth %>% gather(var, value, -year)
...
ggsave(plot = gg, filename = './workspace/Yield_SPEI/png/graphs/yield_spei_pitalito_log.png', units = 'in', width = 7, height = 5, dpi = 300)
...
agu <- agu %>% mutate(spei_log = log10(yield))
agu_gth <- agu %>% dplyr::select(year, -periodo, yield, spei_log)
agu_gth <- agu_gth %>% gather(var, value, -year)
...
ggsave(plot = gg, filename = './workspace/Yield_SPEI/png/graphs/yield_spei_elaguila_log.png', units = 'in', width = 7, height = 5, dpi = 300)
