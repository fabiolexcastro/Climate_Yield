...
spei_12 <- readRDS(file = './rds/spei/spei_12_zones_coffee_v2.rds') %>% filter(year %in% years)

# Apply the function ------------------------------------------------------

# SCALE = 3
tb_pit <- extract_table(tb = spei_03, mp = 'PITALITO')
tb_agu <- extract_table(tb = spei_03, mp = 'EL AGUILA')

saveRDS(object = tb_pit, file = './rds/mpios_vars/tb_pit_03.rds')
saveRDS(object = tb_agu, file = './rds/mpios_vars/tb_agu_03.rds')

# SCALE = 12
tb_pit <- extract_table(tb = spei_12, mp = 'PITALITO')
tb_agu <- extract_table(tb = spei_12, mp = 'EL AGUILA')

saveRDS(object = tb_pit, file = './workspace/Yield_SPEI/rds/mpios_vars/tb_pit_12.rds')
saveRDS(object = tb_agu, file = './workspace/Yield_SPEI/rds/mpios_vars/tb_agu_12.rds')
